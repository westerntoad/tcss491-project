//sceneManager.js holds the state of all unique things for enemies.
...
class SceneManager {
    ...
    load(map) {
        ...
        ASSET_MANAGER.getAsset("./assets/soundtrack/TCSS 491 Phone Call to a New Adventure.mp3").play();
    }
    ...
    cutscene(chapterNumber, cutsceneNumber) {
        console.log("Entered a cutscene");
        this.game.entities = []; //To clear the scene
        if (chapterNumber = 1) {
            if (cutsceneNumber = 0) {
                //Animator Structure: 
                //this.spritesheet = The cutscene you want to use
                //new Animator(this.spritesheet, xStart, yStart, Width, height, 1, frameDuration);
                this.game.addEntity(new Animator());
                //Dialogue Box Support Code
            } if (cutsceneNumber = 1) {
                this.game.addEntity(new Animator());
                //Dialogue Box Support Code
            }
    ...
    //OPTIONAL Chapter 3 Scenes
    chapterThree() {

    };

    //OPTIONAL Chapter 4 Scenes
    chapterFour() {
